...
export default function dependentVariables(state = initialState, action) {
  if (action.type === 'SET_DEP_VARS') {
    return Immutable.fromJS(action.depVars);
  }
  if (action.type === 'CREATE_DEP_VAR') {
    return state.set(action.depVarId, new Immutable.Map({
  ...
  if (action.type === 'ADD_MEASURE') {
    const newMeasure = state.getIn([action.depVarId, 'measures']).push(action.measureId);
