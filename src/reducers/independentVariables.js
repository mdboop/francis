...
export default function independentVariables(state = initialState, action) {
  ...
  if (action.type === 'ADD_INDVAR_OPTION') {
    ...
    let newOptions = state.get(action.indVarId).get('options');
  ...
  if (action.type === 'REMOVE_INDVAR_OPTION') {
    let newOptions = state.get(action.indVarId).get('options');
    // Find index of item
    const optionIndex = newOptions.indexOf(action.option);
    // Remove if in list
    if (optionIndex >= 0) {
